...
async function deleteProject(project: Project) {
  const confirmed = window.confirm(`删除项目“${project.name}”将移除项目、业务事项、清单及附件记录。电脑上的项目文件夹和实际文件会保留，不会删除磁盘资料。确定继续吗？`)
  if (!confirmed) return
  const removed = await run(() => window.workpack.deleteProject(project.id), '项目记录已删除，电脑上的项目文件夹和实际文件已保留')
  if (removed) {
      ...
      <main class="view">
        ...
        <template v-else>
          <section v-if="view === 'home'" class="page-section">
            <div class="page-heading"><div><span class="eyebrow">WORKSPACE OVERVIEW</span><h1>工作台</h1><p>按项目查看事项与文件状态。</p></div><div class="heading-actions"><button class="button secondary" @click="go('templates')">管理模板</button><button class="button primary" @click="openProjectModal()">＋ 新建项目</button></div></div>
