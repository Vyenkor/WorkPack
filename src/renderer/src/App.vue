...
const eventForm = reactive({ projectId: '', templateId: '', name: '', date: new Date().toISOString().slice(0, 10), owner: '' })
...
const activeTemplate = computed(() => snapshot.value.templates.find(template => template.id === templateForm.id) ?? null)

const allEvents = computed(() => snapshot.value.projects.flatMap(project => project.events.map(event => ({ project, event }))))
const pendingRows = computed(() => allEvents.value.flatMap(({ project, event }) => event.items
  .filter(item => gaps(item).length)
  .map(item => ({ project, event, item, gaps: gaps(item) }))))
...
async function run<T>(action: () => Promise<T>, success?: string): Promise<T | false> {
  busy.value = true
  try {
    const result = await action()
    await refresh()
    if (success && result !== false) showToast(success)
    return result
  } catch (error) {
...
function runGlobalSearch() {
  ...
  const row = pendingRows.value.find(item => [item.event.name, item.item.name].some(value => value.toLowerCase().includes(keyword)))
...
function resetEventForm(project: Project) { Object.assign(eventForm, { projectId: project.id, templateId: project.templateIds[0] ?? '', name: '', date: new Date().toISOString().slice(0, 10), owner: project.owner }) }
function openEventModal(project = activeProject.value) { if (!project) return; resetEventForm(project); modal.value = 'event' }
async function saveEvent() {
  if (!eventForm.name.trim() || !eventForm.templateId || !eventForm.owner.trim()) { showToast('请填写事项名称、模板和经办人', 'error'); return }
  const saved = await run(() => window.workpack.createEvent({ ...eventForm }), '业务事项已创建')
  if (!saved) return
  modal.value = null
  const project = snapshot.value.projects.find(item => item.id === eventForm.projectId)
  const event = project?.events.filter(item => item.name === eventForm.name).sort((a, b) => b.date.localeCompare(a.date))[0]
  if (project) openProject(project, event?.id)
}
...
async function saveTemplate() {
  ...
  const saved = await run(() => window.workpack.saveTemplate({ id: templateForm.id || undefined, name: templateForm.name, type: templateForm.type, rules: templateForm.rules }), templateForm.id ? '模板规则已更新' : '模板已创建')
