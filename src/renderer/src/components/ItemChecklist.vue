  ...
  <div class="table-wrap">
    <table class="checklist" aria-label="事项文件清单">
      ...
      <tbody><tr v-for="item in items" :key="item.id" :data-testid="`item-${item.id}`">
        ...
        <td v-for="step in steps" :key="step"><select :value="item.states[step]" :class="['status-select', item.states[step]]" :aria-label="`${item.name}的${stepLabels[step]}状态`" :disabled="busy" @change="change($event, item.id, step)"><option value="pending">待{{ stepLabels[step] }}</option><option value="done" :disabled="step === 'prepared' && !item.attachments.some(file => file.exists)">已{{ stepLabels[step] }}</option><option value="na">不适用</option></select></td>
