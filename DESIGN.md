...
上述界面差异已处理：可读文字至少 12px，辅助文字和控件边界符合对比度要求；类型色与状态色分开；普通面板不使用阴影或悬停位移；清单状态、提示条、弹窗焦点和删除确认已按本规范实现。`src/renderer/src/style.css` 的颜色及字号由文件顶部的 CSS 变量统一管理，变量取值与本文件 token 保持一致。
