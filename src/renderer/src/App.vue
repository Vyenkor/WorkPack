...
const templateForm = reactive({ id: '', name: '', type: '' as BusinessType, rules: [] as Rule[] })
...
const availableTypes = computed(() => [...new Set(snapshot.value.templates.map(template => template.type))])
...
function typeLabel(type: BusinessType) { return businessTypeLabel(type) }
...
const templateTypeInput = computed({
  get: () => templateForm.type ? typeLabel(templateForm.type) : '',
  set: (value: string) => {
    const input = value.trim()
    const builtin = Object.entries(typeLabels).find(([, label]) => label === input)?.[0]
    templateForm.type = builtin ?? input
  }
})
...
<template>
  <div class="app-shell">
    ...
    <div v-if="toast.visible" class="toast" :class="toast.tone" :role="toast.tone === 'error' ? 'alert' : 'status'"><span>{{ toast.tone === 'success' ? '✓' : '!' }}</span>{{ toast.message }}<button v-if="toast.tone === 'error'" class="toast-close" aria-label="关闭错误提示" @click="toast.visible = false">×</button></div>
  </div>
</template>
