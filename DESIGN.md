...
- 窗口最小宽度 1050px。左侧导航固定 240px。内容区最大宽度 1400px，左右内边距 `spacing.xl`（1050–1180px 时改为 `spacing.lg`，见 Responsive Behavior）。
...
- 大于 `xxl`（40px）的间距不能用来制造呼吸感。页面级主要区域之间才允许用到 `xxl`。
...
- 搜索、筛选、排序组成稳定操作组，顺序固定。某一项不存在时，后面的控件前移，不留空位。
- 标题、说明、搜索、筛选各自占一整行不符合本规范。说明文字若必须出现，用 `caption` 跟在标题同一行。
- 工具栏高度 36–44px。内部控件使用现有高度：`button-small` 28px，`button-primary`、`button-secondary`、`text-input` 36px。
...
- 普通数据行 32–40px。
- 工具栏 36–44px。
- 常规按钮和输入框 28–36px，对应现有组件 token，不另设高度。
- 数据行不要做成 48px、56px 或 64px。单行文本使用 `body-sm`，不要为了疏朗再加大行高。
...
- 不要把标题、说明、搜索、筛选拆成多行，也不要为每条记录套一张卡片。
- 不要卡片套卡片，不要用大于 `spacing.xxl` 的留白，也不要用临时 margin 错开同一页的左边界。
- 不要把数据行加到 48px、56px 或 64px 来制造呼吸感。
...
- 窗口变窄时工具栏仍保持横向操作组。必须换行时，搜索、筛选、排序作为一组换行，主要操作仍靠右。不要拆成标题、搜索、筛选各占一行。
...
- 页面布局尚未按 Layout 落地：样式里仍有非 4 倍数间距，工具栏和筛选栏仍纵向拆开，列宽也未按重要性分配。这是后续界面改造，当前页面代码保持原样。
