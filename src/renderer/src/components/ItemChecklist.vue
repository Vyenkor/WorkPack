...
defineProps<{ items: Item[]; busy: boolean }>()
    ...
    <table class="checklist" aria-label="事项文件清单">
      ...
      <tbody><tr v-for="item in items" :key="item.id" :data-testid="`item-${item.id}`">
        <td class="document-cell"><strong>{{ item.name }}</strong><p v-if="item.note" class="help">{{ item.note }}</p>
          <div class="tags"><span v-for="gap in gaps(item)" :key="gap" class="tag orange">{{ gap }}</span><span v-if="!gaps(item).length" class="tag green">已完成</span></div>
        ...
        <td><div class="stack-actions"><button class="link" :disabled="busy" @click="emit('edit', item)">编辑要求</button><button class="link danger" :disabled="busy" @click="emit('remove', item)">删除清单项</button></div></td>
      </tr></tbody>
    </table>
