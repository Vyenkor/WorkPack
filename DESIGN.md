...
- 内容本身超过一行时，行高随内容增高，不受 40px 限制。这包括两行文件名、错误信息、辅助 metadata、Badge 与操作按钮叠放、多行状态说明。
...
- 页面布局尚未按 Layout 落地，后续界面改造再动，当前页面代码保持原样：
  ...
  - 列宽尚未按重要性分配。
  - 单行 `table-row` 大约 45px，尚未收紧到 32–40px。收紧时只改单行行高，两行文件名、错误信息、辅助 metadata、Badge 与操作按钮、多行状态说明仍随内容增高。
